  ...
  <div class="js-format">
    ...
    <n-card>
      ...
      <div class="button-group">
        ...
        <!-- 下载按钮 -->
        <n-button @click="downloadJs" :disabled="!input.trim()">
          {{ t('format.js.download') }}
        </n-button>
      </div>
      <!-- 错误提示 -->
...
const downloadJs = () => {
  ...
  try {
    error.value = ''
    // 创建Blob对象
    const blob = new Blob([input.value], { type: 'text/javascript;charset=utf-8' })
    // 创建下载链接
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `formatted-script.js`
    // 触发下载
    ...
    message.success(t('format.js.downloadSuccess'))
  } catch (e) {
    error.value = e.message
