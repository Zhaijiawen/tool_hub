  ...
  <div class="sql-format">
    ...
    <n-card>
      ...
      <div class="button-group">
        ...
        <!-- 下载按钮 -->
        <n-button @click="downloadSql" :disabled="!input.trim()">
          {{ t('format.sql.download') }}
        </n-button>
      </div>
      <!-- 错误提示 -->
...
const downloadSql = () => {
  ...
  try {
    error.value = ''
    // 创建Blob对象
    const blob = new Blob([input.value], { type: 'text/x-sql;charset=utf-8' })
    // 创建下载链接
    ...
    message.success(t('format.sql.downloadSuccess'))
  } catch (e) {
    error.value = e.message
