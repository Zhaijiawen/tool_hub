    ...
    <n-card :title="t('encrypt.ecdh.title')">
      <n-form>
        <n-form-item :label="t('encrypt.ecdh.privateKey')">
          <n-input v-model:value="privateKey" type="textarea" :placeholder="t('encrypt.ecdh.privateKeyPlaceholder')"
            :autosize="{ minRows: 3, maxRows: 10 }" />
        </n-form-item>
        <n-form-item :label="t('encrypt.ecdh.publicKey')">
          <n-input v-model:value="publicKey" type="textarea" :placeholder="t('encrypt.ecdh.publicKeyPlaceholder')"
            :autosize="{ minRows: 3, maxRows: 10 }" />
        </n-form-item>
        <n-form-item :label="t('encrypt.ecdh.peerPublicKey')">
          <n-input v-model:value="peerPublicKey" type="textarea"
            :placeholder="t('encrypt.ecdh.peerPublicKeyPlaceholder')" :autosize="{ minRows: 3, maxRows: 10 }" />
        </n-form-item>
        <n-space>
