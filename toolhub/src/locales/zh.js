...
\`\`\``,
  ...
  image: {
    convert: {
      ...
      infoTitle: '格式说明',
      infoContent: '<b>有损格式 (JPEG, WebP):</b> 体积小，适合照片，但会损失部分质量。<br/><b>无损格式 (PNG, GIF):</b> 完美保留所有细节，适合需要高质量的图像，但体积较大。<br/><br/><b>注意:</b> 由于浏览器重新编码，即使质量设为100%，输出文件有时也可能大于原始文件。'
    },
    rotate: {
      title: '图片旋转',
      upload: '上传图片',
  ...
  other: {
    ...
    ip: {
      ...
      myIP: '本机 IP',
      ip: 'IP 地址',
      ipPlaceholder: '请输入要查询的 IP 地址，如 8.8.8.8',
      ipRequired: '请输入 IP 地址',
      invalidIP: '请输入有效的 IP 地址',
      domain: '域名',
      domainPlaceholder: '请输入要反查的域名，如 google.com',
      ...
      timezone: '时区',
      publicIP: '公网 IP',
      lookupResult: 'IP 查询结果',
      reverseResult: 'IP 反查结果',
      myIPResult: '本机 IP 信息',
      getMyIP: '获取本机 IP',
      lookupSuccess: 'IP 查询成功',
      reverseSuccess: 'IP 反查成功',
      getMyIPSuccess: '获取本机 IP 成功',
      lookupError: 'IP 查询失败',
      reverseError: 'IP 反查失败',
      getMyIPError: '获取本机 IP 失败',
      infoTitle: '使用说明',
      infoContent: 'IP 查询：输入 IP 地址，查询地理位置、ISP 等信息。IP 反查：输入域名，查询对应的 IP 地址列表。本机 IP：获取您当前的公网 IP 地址和位置信息。注意：本工具为演示版本，数据仅供参考。',
      demoMode: '演示模式',
      demoModeDesc: '当前为演示模式，显示的是模拟数据，实际使用需要后端 API 支持'
    },
    calculator: {
      title: '计算器',
      description: '在线计算器',
